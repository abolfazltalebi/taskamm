...
export const WeeklyReviewView: React.FC = () => {
  ...
  return (
    <div id="weekly-review-view-container" className="space-y-6">
      {/* Header Banner */}
      <div className="bg-white dark:bg-[#131F2E] p-6 sm:p-7 rounded-3xl border border-gray-200/90 dark:border-slate-800 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-300 text-xs font-bold border border-emerald-200/70 dark:border-emerald-500/30 flex items-center gap-1">
              <Trophy className="w-3.5 h-3.5 text-amber-500" />
              <span>گزارش و مرور هفته</span>
            </span>
            <span className="text-xs text-gray-500 dark:text-slate-400 font-medium">{formatJalaliDate(now, 'dayOfWeek')}</span>
          </div>
          <h2 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white">
            این هفته چه کارهایی به سرانجام رسید؟
          </h2>
          <p className="text-xs sm:text-sm text-gray-600 dark:text-slate-400 mt-1 font-medium">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white font-bold text-xs shadow-xs transition self-start md:self-auto cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-[#131F2E] p-5 rounded-3xl border border-gray-200/90 dark:border-slate-800 shadow-2xs space-y-1">
          <span className="text-xs text-gray-500 dark:text-slate-400 flex items-center gap-1.5 font-bold">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-[#131F2E] rounded-3xl border border-gray-200/90 dark:border-slate-800 p-5 sm:p-6 space-y-4 shadow-2xs">
          ...
          <div className="space-y-2.5 max-h-[320px] overflow-y-auto pe-1">
            {thisWeekCompleted.length > 0 ? (
              thisWeekCompleted.map((t) => (
                <div
                  key={t.id}
                  className="flex items-center justify-between gap-3 p-3 rounded-2xl bg-gray-50/80 dark:bg-[#0B131E]/60 border border-gray-200/80 dark:border-slate-800 text-xs"
                >
            ...
            {rolloverTasks.length > 0 ? (
              rolloverTasks.map((t) => (
                ...
                >
                  <span className="font-semibold text-gray-800 dark:text-slate-200 truncate">{t.title}</span>
                  {t.dueAt && (
                    <span className="text-rose-600 dark:text-rose-400 text-[11px] shrink-0 font-bold">
